...
from .models import (
    MainContract,
    MainContractBudgetCostTable,
    MainContractDetail,
    MainContractInvoiceDetailsTable,
    MainContractInvoiceTable,
    SubContractDetail,
    SubContractInvoiceDetailsTable,
    SubContractInvoiceTable,
    SubContractTable,
)
...
def save_contract(request):
    ...
    if request.method == "POST":
        ...
        if not rows:
            return JsonResponse(
                {"status": "error", "message": "No rows provided"}, status=400
            )
        ...
        if not company_details_record:
            return JsonResponse(
                {"status": "error", "message": "No company details found for the user"},
                status=400,
            )
        ...
        for row in rows:
            ...
            if len(data) >= 12:  # Ensure there are enough elements in the data list
                # try:
                get_estimated_assembly = Estimation_Assemblies_Table.objects.filter(
                    id=data[14]
                ).last()
                ...
                filter_Estimation_Assemblies_Resource_Details = (
                    Estimation_Assemblies_Resource_Details_Table.objects.filter(
                        Estimation_Assemblies=get_estimated_assembly
                    )
                )
                for estimated_assembly in filter_Estimation_Assemblies_Resource_Details:
                    var_MainContractBudgetCost = MainContractBudgetCostTable(
                        ...
                        budget_cost=float(
                            estimated_assembly.Resource_record.Budget_Unit_Cost
                        )
                        * int(clean_decimal(data[4])),
                    )
...
def create_invoice(request):
    if request.method == "POST":
        ...
        for cd_id, inv_qty in zip(contract_details_id, invoice_qty):
            ...
            try:
                get_contract_details = MainContractDetail.objects.get(
                    id=contract_details_id
                )
            except Exception as exc:
                print(exc)
                messages.error(request, f"Error : {exc}")
            ...
            if float(get_remain_invoice) < float(invoice_quantity):
                messages.error(
                    request,
                    f"Error : Your remain invoice quantity is {get_remain_invoice}, you trying to add {invoice_quantity}",
                )
                return redirect("view_contract", contract_id)
            ...
            var_MainContractInvoice_details = MainContractInvoiceDetailsTable(
                ...
                Invoice_Revenue=float(get_contract_details.unit_price)
                * float(invoice_quantity),
                Next_Remain_Quantity=Next_Remain_Quantity,
                Next_Remain_Revenue=float(get_contract_details.unit_price)
                * float(Next_Remain_Quantity),
            )
...
def view_contract(request, contract_id):
    ...
    filter_stores = StoreTable.objects.filter(
        contract_value=contract, stock_trasaction_status="Stock-Out"
    )
    ...
    Sum_Total_Actual_Resources_Cost = sum(
        detail.Total_Actual_Resources_Cost() for detail in contract_details
    )

    main_contract_budget_costs = MainContractBudgetCostTable.objects.filter(
        contract_details__in=contract_details
    )
    sum_contract_budget_cost = sum(
        float(budget_cost_row.budget_cost)
        for budget_cost_row in main_contract_budget_costs
    )
    ...
    filter_resoures_level_1 = Resource_Code_L1_Table.objects.filter(
        Company_Details=contract.company_details
    )
...
def create_contract(request):
    ...
    context = {
        "assemblies_data": json.dumps(
            assemblies_data
        ),  # Pass as JSON for easy JS access
    }
    return render(request, "contractApp/create_contract.html", context)
...
def delete_contract_row(request, row_id):
    ...
    if request.method == "DELETE":
        try:
            ...
            return JsonResponse(
                {"status": "error", "message": "Row not found."}, status=404
            )
...
def Contract_Management_pro(request):
    ...
    for main_contract in my_contracts:
        ...
        level1_data = {"contract": main_contract, "level1": []}
        for level1 in level1_assemblies:
            level2_assemblies = Assemblies_Code_L2_Table.objects.filter(
                Assemblies_Code_L1=level1
            )
            ...
            level1_data["level1"].append({"level1": level1, "level2": level2_list})
        data.append(level1_data)
    # print(data)
    for dat in data:
        print(dat)
...
def Contract_Management_main(request):
    ...
    filter_assemblies_level_1 = Assemblies_Code_L1_Table.objects.filter(
        Company_Details=company_details_record
    )
    context = {
...
def contract_assembly_resource_details_management(request):
    ...
    return render(
        request,
        "contractApp/contract_assembly_resource_details_management.html",
        context,
    )
...
def create_subcontract(request):
    ...
    filter_MainContract_query = MainContract.objects.filter(
        company_details=company_details_record
    )
    filter_MainContract_list = list(
        filter_MainContract_query.values("id", "contract_name")
    )
...
def save_subcontract(request):
    if request.method == "POST":
        ...
        print(
            "hiddencontractInputId, hiddenassemblyInputId, totalTotalPrice, subcontract_name"
        )
        print(
            hiddencontractInputId,
            hiddenassemblyInputId,
            totalTotalPrice,
            subcontract_name,
        )
        ...
        for row in rows:
            ...
            # if len(data) >= 12:  # Ensure there are enough elements in the data list
            # try:
            get_subcontract_detail_row = Estimation_Assemblies_Table.objects.filter(
                id=data[6]
            ).last()
...
def save_edit_subcontract(request):
    if request.method == "POST":
        ...
        sub_contract.contract_value = MainContract.objects.filter(
            id=hiddencontractInputId
        ).last()
...
def subcontract_create_invoice(request):
    if request.method == "POST":
        ...
        for cd_id, inv_qty in zip(contract_details_id, invoice_qty):
            ...
            try:
                get_contract_details = SubContractDetail.objects.get(
                    id=contract_details_id
                )
            except Exception as exc:
                print(exc)
            ...
            var_MainContractInvoice_details = SubContractInvoiceDetailsTable(
                ...
                Invoice_Quantity=invoice_quantity,
                Invoice_Revenue=float(get_contract_details.unit_cost)
                * float(invoice_quantity),
                Next_Remain_Quantity=Next_Remain_Quantity,
                Next_Remain_Revenue=float(get_contract_details.unit_cost)
                * float(Next_Remain_Quantity),
            )
            var_MainContractInvoice_details.save()
